...
void boid::calc_acc_all(){
    ...
    for (i = 0; i < nboids; i++){
        for (j = 0; j < nboids; j++){
            if (j == i) continue;
            // printf("Thread %d of %d - i = %d and j = %d at time %f\n", omp_get_thread_num(), omp_get_max_threads(), i, j, time);
            vec3 diff = (pos[i] - pos[j]);
            float distsq = diff.normsqrd();
            if (distsq < gtfo_distance * gtfo_distance){
                collision[i] += (diff * gtfo_distance / distsq - diff / diff.norm()) * gtfo_distance;
                counter[i] += 1;
            }
        }
    }
    ...
    for (i = 0; i < nboids; i++){
        if (counter[i] > 0) collision[i] /= counter[i];
    }

    // alignment
    vec3* avg_vel = new vec3[nboids];
    alignment = new vec3[nboids];
    int* alignment_counter = new int[nboids]{0};
    ...
    #pragma omp parallel for private(i)
    for (i = 0; i < nboids; i++){
        avg_vel[i] = avg_vel[i] / ((float)alignment_counter[i]+1);
    ...
    int* centering_counter = new int[nboids]{0};
    ...
    #pragma omp parallel for private(i)
    for (i = 0; i < nboids; i++){
        avg_pos[i] = avg_pos[i] / ((float)centering_counter[i]+1);
